...
public sealed class AdditionalJsonTests
{
    private static CancellationToken Ct => TestDeadlines.Wrap(TestContext.Current.CancellationToken);
    public static TheoryData<Dialect> Dialects => AdditionalEdgeTests.Dialects;
    ...
    public async Task F8_MutationBetweenPagesDoesNotSkipUntouchedEntries(Dialect dialect)
    {
        ...
        while (true)
        {
            byte[] bytes = await AdditionalEdgeTests.DirectoryPage(a, directory, offset, 4096 - Constants.IOHDRSZ);
            if (bytes.Length == 0)
            {
                break;
            }
        ...
        byte[] first = await AdditionalEdgeTests.DirectoryPage(a, gone, 0, 1000);
        ...
        Assert.Empty(await AdditionalEdgeTests.DirectoryPage(a, gone, resume, 1000));
    ...
    public async Task F12c_CapRejectsCreateWriteAndRenameWithoutChangingOpenObjects(Dialect dialect)
    {
        ...
        await AdditionalEdgeTests.Error(Errno.ENOSPC, async () => await s.MkdirAsync("new", 0x1ED, Ct));
        await AdditionalEdgeTests.Error(Errno.ENOSPC, async () => await open.WriteAsync(0, "big"u8.ToArray(), Ct));
        await AdditionalEdgeTests.Error(Errno.ENOSPC, async () => await s.RenameAsync("a", "longer", Ct));
        // Quotes expand on serialization: a two-byte value can cost more than the old one.
        await AdditionalEdgeTests.Error(Errno.ENOSPC, async () => await open.WriteAsync(0, "\"\""u8.ToArray(), Ct));
    ...
    public async Task F12c_ExactSerializedThresholdAndConcurrentGrowth(Dialect dialect)
    {
        ...
        await AdditionalEdgeTests.Error(Errno.ENOSPC, async () => await first.WriteAsync(2, "c"u8.ToArray(), Ct));
    ...
    public async Task F28_Utf8CharactersMayCrossWriteBoundariesAndInvalidBytesAreAtomic(Dialect dialect)
    {
        foreach (string value in new[] { "é", "世", "🚀" })
        {
            ...
            for (int split = 1; split < bytes.Length; split++)
            {
                ...
                byte[] before = Serialize(tree);
                await AdditionalEdgeTests.Error(Errno.EINVAL, async () => await file.WriteAsync(0, new byte[] { 0xFF }, Ct));
                Assert.Equal(before, Serialize(tree));
            }
        }
    }
    ...
    public async Task F28_IncompleteUtf8AtClunkIsAnErrorAndFidIsReleased(Dialect dialect)
    {
        ...
        await AdditionalEdgeTests.Error(Errno.EINVAL, async () => await s.Messages.ClunkAsync(new Tclunk(0, file.Fid), Ct));
        await AdditionalEdgeTests.Error(Errno.EBADF, async () => await s.Messages.ClunkAsync(new Tclunk(0, file.Fid), Ct));
    ...
    public async Task F29_CreateAndMoveCannotExceedReloadableDepth(Dialect dialect)
    {
        ...
        await using ServerHarness h = await ServerHarness.StartAsync(filesystem: new JsonFilesystem(tree, writable: true));
        await using NinePSession s = await h.ConnectAsync(dialect);
        await AdditionalEdgeTests.Error(Errno.ENOSPC, async () => await s.MkdirAsync("a/b/c", 0x1ED, Ct));
        if (dialect == Dialect.P9_2000_L)
        {
            await AdditionalEdgeTests.Error(Errno.ENOSPC, async () => await s.RenameAsync("move", "a/b/move", Ct));
        }
        Assert.Equal(before, Serialize(tree));
    ...
    public async Task F30_WriteBackFailureReportsErrorAndLeavesACompleteDocument(bool afterReplace)
    {
        ...
        try
        {
            ...
            await AdditionalEdgeTests.Error(Errno.EIO, async () => await file.WriteAsync(0, "after!"u8.ToArray(), Ct));
