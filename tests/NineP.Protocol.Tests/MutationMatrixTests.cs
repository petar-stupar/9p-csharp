...
public sealed class MutationMatrixTests
{
    ...
    private static IEnumerable<Func<bool>> ClaimAttempts()
    {
        ...
        byte[] rread = Payload(MessageType.Rread, int.MaxValue);
        byte[] rreaddir = Payload(MessageType.Rreaddir, int.MaxValue);
    ...
    private static byte[] Payload(MessageType type, int count)
    {
...
internal static class Mutations
{
    ...
    public static IEnumerable<string> ApplicableTo(WireVector vector)
    {
        ...
        if (vector.Type is MessageType.Twrite)
        {
