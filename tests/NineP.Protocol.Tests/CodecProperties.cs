...
{
    private const int CorpusSize = 77;
    ...
    public void TwritePayloadRoundTrips() =>
        Prop.ForAll(Tags(), Offsets(), Payloads(), (tag, offset, data) =>
        {
            ...
            Twrite decoded = MessageCodec.Decode<Twrite>(Encode(in message, Dialect.P9_2000), Dialect.P9_2000);
            return decoded.Data.Span.SequenceEqual(data);
        }).QuickCheckThrowOnFailure();
    ...
    public void EncodeAfterDecodeIsByteIdentical() =>
        Prop.ForAll(VectorIndexes(), index =>
        {
            ...
            return vector.Name.Contains('(', StringComparison.Ordinal)
                || VectorCodec.ReEncode(vector).SequenceEqual(vector.ToBytes());
        }).QuickCheckThrowOnFailure();
    ...
    private static Arbitrary<ulong> Offsets() =>
        Arb.From(Gen.Choose(0, int.MaxValue).Select(i => (ulong)i));
