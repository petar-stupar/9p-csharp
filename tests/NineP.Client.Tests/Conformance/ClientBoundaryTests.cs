...
public sealed class ClientBoundaryTests
{
    ...
    [Fact]
    public async Task AnEmptyWriteAllSendsNoTwrite()
    {
        await using Harness h = await Harness.StartAsync();
        await h.Fid.WriteAllAsync(ReadOnlyMemory<byte>.Empty, Ct);
        // The next frame must be the sentinel Tgetattr, so even an empty Twrite is detected.
        await h.SentinelAsync();
    }

    [Fact]
    public async Task AZeroProgressOrdinaryWriteIsEio()
    {
        await using Harness h = await Harness.StartAsync(new ClientOptions { InFlightWindow = 2 });
        Task writing = h.Fid.WriteAllAsync("nonempty"u8.ToArray(), Ct).AsTask();
        Twrite request = await h.Server.ReadAsync<Twrite>(Ct);
        Assert.NotEmpty(request.Data.ToArray());
        await h.Server.WriteAsync(new Rwrite(request.Tag, 0), Ct);
        await Error(Errno.EIO, async () => await writing.WaitAsync(Ct));
        await h.SentinelAsync();
    }
    ...
    public async Task APageEndingOnARecordBoundaryContinues(Dialect dialect)
    {
        await using Harness h = await Harness.StartAsync(new ClientOptions { Dialects = [dialect] }, dialect);
        await using IAsyncEnumerator<DirEntry> iterator = h.Fid.ReadDirAsync(Ct).GetAsyncEnumerator(Ct);
        ulong offset = 0;
        for (int i = 1; i <= 3; i++)
        {
            ...
            {
                Treaddir request = await h.Server.ReadAsync<Treaddir>(Ct);
                Assert.Equal(offset, request.Offset);
                page = Entries((ulong)i);
                await h.Server.WriteAsync(new Rreaddir(request.Tag, page), Ct);
                offset = (ulong)i;
            }
            else
            {
                Tread request = await h.Server.ReadAsync<Tread>(Ct);
                Assert.Equal(offset, request.Offset);
                System.Buffers.ArrayBufferWriter<byte> frame = new();
                Rstat stat = new(0, new StatRecord { Name = "entry", Uid = "", Gid = "", Muid = "", Extension = "", Length = 0 });
                NineP.Protocol.Codec.MessageCodec.Encode(frame, in stat, dialect);
                page = frame.WrittenSpan[9..].ToArray();
                await h.Server.WriteAsync(new Rread(request.Tag, page), Ct);
                offset += (ulong)page.Length;
            }
            Assert.True(await next);
            Assert.Equal("entry", iterator.Current.Name);
        }
        Task<bool> end = iterator.MoveNextAsync().AsTask();
        if (dialect == Dialect.P9_2000_L)
        {
            Treaddir request = await h.Server.ReadAsync<Treaddir>(Ct);
            Assert.Equal(offset, request.Offset);
            await h.Server.WriteAsync(new Rreaddir(request.Tag, ReadOnlyMemory<byte>.Empty), Ct);
        }
        else
        {
            Tread request = await h.Server.ReadAsync<Tread>(Ct);
            Assert.Equal(offset, request.Offset);
            await h.Server.WriteAsync(new Rread(request.Tag, ReadOnlyMemory<byte>.Empty), Ct);
        }
        Assert.False(await end);
    }
    ...
    private sealed class Harness(NinePSession session, FakeNinePServer server, NinePFid fid) : IAsyncDisposable
    {
        ...
        public static async Task<Harness> StartAsync(ClientOptions? options = null, Dialect dialect = Dialect.P9_2000_L)
        {
            var (connection, server) = FakeNinePServer.CreatePair();
