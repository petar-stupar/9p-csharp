    ...
    public async Task LinuxV9fsAgainstOurJsonfs(string version)
    {
        ...
        try
        {
            ...
            string script = string.Format(
                ...
                + "cd /mnt/ninep-interop; ls; echo ---; cat name; echo; cat unicode; echo; cat dir/file.txt; echo; "
                + "stat -c '%s %F' name; (echo x > name) 2>&1 || true; cd /; umount /mnt/ninep-interop",
                port, version, host);
            ...
            Assert.Contains("name", lines[separator + 5], StringComparison.Ordinal); // the refused write, whatever wording the kernel gives it
        }
        finally
        {
            Stop(server);
