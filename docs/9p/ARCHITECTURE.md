...
8. Docs: `README.md` (install, 60-second client, 60-second server), `docs/protocol.md` (vendored
   ...
   `docs/security.md`, `docs/mounting.md` (§14), `CHANGELOG.md`, API docs generated by the
   ecosystem tool.
...
## 14. What a server owes a kernel client (owner decision, 2026-09-11)

Every port ships a `docs/mounting.md`. The suite proves a server talks to *9P clients*; this is
about the other thing a user does with it, and the two are not the same exercise.

**Mounting is not interop.** Linux mounts 9P natively through `v9fs`. **macOS and Windows cannot
mount it at all** — macOS's `/sbin/mount_9p` mounts a Virtualization.framework share by tag from
inside a guest and cannot address a TCP server, and Windows has no v9fs. On both, the only route is
a bridge: a Linux container mounts the tree over 9P and re-exports it over SMB. Since most users of
most of these ports are on one of those two platforms, the bridge is the common case, not an exotic
...
1. **`Tstatfs` must be answered.** Samba calls `disk_free` on tree connect. A server that answers
   `EOPNOTSUPP` fails the SMB mount, and the host reports `Operation not supported` with nothing
   naming 9P; only the Samba log says why. The numbers need not be interesting.
2. **A write-only file cannot be opened.** `Txattrwalk` requires read permission before the server
   is consulted for an xattr handler, and Samba asks for extended attributes on open. So mode
   `0222` — the Plan 9 convention — is refused `EACCES` on *open*. Anything meant to be written
   through a bridge needs a read bit, whether or not reading it means anything.
3. **A redirect is an attribute change, not just a write.** `>` opens with `O_TRUNC`, which v9fs
   sends as one request carrying size *and* mtime. A handler that refuses every attribute update —
   the obvious thing for a synthetic tree — fails the open before a byte is written. The core's
   half of this is §8 rule 43.
4. **The attach identity is `nobody`/`-1` unless the mount says otherwise**, so by default no
...
macOS gating network volumes per application — is host-shaped rather than language-shaped, but
every port's users meet it, so `docs/mounting.md` carries it too. The C# port's is the model.
...
| 2026-09-11 | Every port ships `docs/mounting.md`; four re-export obligations are language-neutral (§14) | Bug [bugs/002](bugs/002-re-exported-tree-caveats.md). macOS and Windows cannot mount 9P at all, so a bridge is the common case for most users of most ports, and the suite does not exercise it. Lifted to the reference rather than left in the C# docs so the other thirteen do not each rediscover it. |
